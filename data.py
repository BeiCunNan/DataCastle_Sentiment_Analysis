...
def load_dataset(tokenizer, train_batch_size, test_batch_size, model_name, method_name, workers):
    data = pd.read_csv('./data/treat_train.csv', sep=None, header=0, encoding='utf-8', engine='python')
    ...
    submit_set = MyDataset(sub_sen, sub_lab, method_name, model_name)
    # DataLoader
    ...
    test_loader = DataLoader(test_set, batch_size=test_batch_size, shuffle=True, num_workers=workers,
                             collate_fn=collate_fn, pin_memory=True)
    submit_loader = DataLoader(submit_set, batch_size=train_batch_size, shuffle=True, num_workers=workers,
                               collate_fn=collate_fn, pin_memory=True)
